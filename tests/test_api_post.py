...
class TestPostAPI:
    ...
    def test_post_create(self, user_api_client, user, another_user, group_1):
        ...
        test_data = response.json()
        msg_error = (
            'Check that when posting at `/API/V1/Posts/` the dictionary with the data of the new article is returned'
        )
    ...
    def test_post_patch_current(self, user_api_client, post, another_post):
        response = user_api_client.patch(f'/api/v1/posts/{post.id}/',
        ...
        response = user_api_client.patch(f'/api/v1/posts/{another_post.id}/',
                                     data={'text': 'Поменяли текст статьи'})
