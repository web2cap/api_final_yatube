...
class TestFollowAPI:
    ...
    def test_follow_create(
        self, user_api_client, follow_2, follow_3, user, user_2, another_user
    ):
        ...
        msg_error = "Check that when posting at `/API/V1/Follow/` the dictionary with the data of the new subscription is returned"
    ...
    def test_follow_search_filter(
        ...
        assert (
            len(response.json()) == follow_user.filter(following=another_user).count()
        ), (
            "Check that when a GET request with the `Search` parameter on`/API/V1/Follow/`"
            "The result of the subscription search is returned"
        )
