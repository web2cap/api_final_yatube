...
class TestComment:
    ...
    def test_comment_model(self):
        ...
        assert text_field is not None, 'Add the name of the event `Text` Model` Comment` '
        ...
        assert author_field is not None, 'Add a user, an author who created an event `Author` Model` Comment` '
    ...
    def test_comment_add_view(self, client, post):
        try:
            ...
            assert False, f'''The `/posts/<Post_ID>/comment/` page works incorrectly.Error: `{e}` '''
    ...
    def test_comment_add_auth_view(self, user_client, post):
        try:
            ...
            assert False, f'''The `/posts/<Post_ID>/comment/` page works incorrectly.Mistake:`{e}`'''
        if response.status_code in (301, 302) and response.url == f'/posts/{post.id}/comment/':
            url = f'/posts/{post.id}/comment/'
        else:
            url = f'/posts/{post.id}/comment'
