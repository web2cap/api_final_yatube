...
class TestFollow:
    ...
    def test_follow(self, field_name):
        ...
        assert checking_field.remote_field.related_name == related_name, (
            f'{field_in_model_text} must contain '
        ...
        assert checking_field.remote_field.on_delete.__name__ == 'CASCADE', (
            f'{field_in_model_text} must provide '
    ...
    def check_url(self, client, url, str_url):
        ...
        if response.status_code in (301, 302) and response.url == f'{url}/':
            response = client.get(f'{url}/')
        assert response.status_code != 404, f'Page `{str_url}` not found, check this address in *urls.py*'
        return response
    ...
    def test_follow_auth(self, user_client, user, post):
        ...
        assert user.follower.count() == 0, 'Check what subscriptions are considered correctly'
        self.check_url(user_client, f'/profile/{post.author.username}/follow', '/profile/<username>/follow/')
        assert user.follower.count() == 0, 'Check that you cannot subscribe to yourself '
        ...
        self.check_url(user_client, f'/profile/{user_1.username}/follow', '/profile/<username>/follow/')
        assert user.follower.count() == 1, 'Check that you can subscribe to the user only once '
        ...
        assert user.follower.count() == 1, 'Check that you can unsubscribe from the user '
        response = self.check_url(user_client, '/follow', '/follow/')
        ...
        assert user.follower.count() == 0, 'Check that you can unsubscribe from the user '
        response = self.check_url(user_client, '/follow', '/follow/')
